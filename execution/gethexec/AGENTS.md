...
- 本地 gRPC 日志流由 `logstream_grpc.go` 管理，固定监听 `0.0.0.0:18888`，服务方法为 `nitro.logs.v1.LogStream/Subscribe`。请求为 `google.protobuf.Empty`，每条响应为包含一个 EVM log 的 `google.protobuf.Struct`，其中 `sender` 是交易发起者。
